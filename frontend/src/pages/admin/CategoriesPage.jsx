...
const CategoriesPage = () => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || t('admin.failed_delete_category'));
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-900">{t('admin.categories', 'Categories')}</h1>
        <button
          onClick={handleAdd}
              ...
              {categories.map((category) => (
                <tr key={category._id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">{category.nameFr}</div>
              ...
              {categories.length === 0 && (
                <tr>
                  <td colSpan="3" className="px-0 py-0">
                    <EmptyState 
                      title={t('admin.no_categories')} 
                      description={t('admin.no_categories_desc')}
                    />
                  </td>
                </tr>
              )}
      ...
      <ConfirmDialog
        ...
        message={
          error ? (
            ...
            t('admin.confirm_delete_category', { name: selectedCategory?.nameFr || selectedCategory?.nameAr })
          )
        }
